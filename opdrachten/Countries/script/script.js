...
function normalizeCountry(myCountry) {
    return myCountry.map(country =>{
        ...
        const avgIncome = country.income.toFixed(2) + "avgIncome"

        const population = parseInt(country.population)

        const lifeExpectancy = country.lifeExpectancy.toFixed(2)

        return {name, region, avgIncome, population, lifeExpectancy}
...
function southAsia(myCountry){

    return myCountry.region === "South Asia"

}
// calculate the total population of South Asia
// southAsianPopulation =
function southAsiaPopulation(total, myCountry){

    return total + myCountry.population 

}
// calculate the maximum life expectancy of a country in South Asia
// soutAsianMaxLifeExpectancy =
function southAsiaMaxLifeExpectancy(total, myCountry){

    return Math.max(myCountry.lifeExpectancy)

}

const newdata = normalizeCountry(countries)
southAsianCountries = normalizeCountry(countries).filter(southAsia)
southAsianPopulation = normalizeCountry(countries).filter(southAsia).reduce(southAsiaPopulation, 0)
southAsianMaxLifeExpectancy = normalizeCountry(countries).filter(southAsia).reduce(southAsiaMaxLifeExpectancy, 0)
